...
const singleReplaceFields = {
  oldText: Type.Optional(replaceFields.oldText),
  newText: Type.Optional(replaceFields.newText),
  // old_string: Type.Optional(replaceFields.old_string),
  // new_string: Type.Optional(replaceFields.new_string),
} as const;
...
interface NormalizedReplaceEdit {
  oldText: string;
  newText: string;
}
...
type MatchedEdit = NormalizedReplaceEdit & {
  index: number;
  matchLength: number;
};
...
function normalizeReplaceEdit(
  ...
  path: string,
  scope: string,
): NormalizedReplaceEdit {
  const oldText = edit.oldText; // ?? edit.old_string;
  const newText = edit.newText; // ?? edit.new_string;

  if (oldText === undefined || newText === undefined) {
    throw new Error(
      `Edit tool input is invalid. ${scope} in ${path} requires both oldText and newText.`,
    );
  }
...
function getMultiReplaceModeInput(
  ...
  return {
    ...
    edits: input.edits.map((edit, index) =>
      normalizeReplaceEdit(edit, input.path, `edits[${index}]`),
    ),
  };
}
...
async function generateDiffString(
  ...
  for (const line of lines) {
    ...
    if (oldLine && newLine) {
      oldLineNum = Number.parseInt(oldLine, 10);
      newLineNum = Number.parseInt(newLine, 10);
